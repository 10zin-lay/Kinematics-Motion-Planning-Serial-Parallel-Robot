...
function [thetaA, thetaB, legLength] = InverseKinematics(basePoint, platformPoint, linkA, linkB)
    ...
    thetaA = angleBaseToPlatform + angleOffset;
    thetaB = angleBaseToPlatform - angleOffset;
end
...
orientationValues = 0:pi/1000:360*pi/180;
centerXValues = ones(length(orientationValues)) * 0;
centerYValues = ones(length(orientationValues)) * 0;
...
for k = 1:length(centerXValues)
    centerX = centerXValues(k);
    centerY = centerYValues(k);
    orientation = orientationValues(k);
    ...
    set(trajectoryPlot, 'XData', centerX, 'YData', centerY);
